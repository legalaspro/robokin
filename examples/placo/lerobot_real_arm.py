...
"""SO-101 Placo + LeRobot — real-arm demo (single control loop).
...
Usage:
    python examples/placo/lerobot_real_arm2.py
    python examples/placo/lerobot_real_arm2.py --pid 15 0 5
    python examples/placo/lerobot_real_arm2.py --port /dev/so101_follower
"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="SO-101 Placo + LeRobot (control loop)")
    ...
    # ── Hardware ──────────────────────────────────────────────────────
    arm = SO101LeRobotArm(SO101LeRobotConfig(port=args.port, calibration_id=args.calibration_id))
    arm.connect()
    ...
    print("\n═══════════════════════════════════════════════════════════════")
    print("  robokin Placo + LeRobot (control loop demo)")
    ...
    try:
        while True:
            ...
            if segment_active:
                ...
                q_cmd = solver.servo_step(q_meas, T_ref)
