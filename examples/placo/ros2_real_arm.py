...
"""ROS2 example: Placo IK + Viser for SO-101 follower arm.
...
Prerequisites:
  ...
  3. Launch the follower arm with vision:
     ...
     This publishes the following topics:
       ...
       - /follower/image_raw             (sensor_msgs/Image)  [wrist camera]
       - /static_camera/image_raw        (sensor_msgs/Image)  [overhead camera]

Then run this example:
  python ros2_real_arm.py

To enable camera feeds in the Viser UI, pass use_cameras:=true:
  python ros2_real_arm.py --ros-args -p use_cameras:=true

Integrates with so101-ros-physical-ai stack:
  - Subscribes to /follower/joint_states for real arm feedback
  - Subscribes to camera topics for display in Viser (when use_cameras is true)
