...
Provides two planning strategies for moving from a start configuration
to a goal end-effector pose:

- **cartesian**: Interpolates in Cartesian space using the solver's
  ``generate_segment()`` — the end-effector follows a straight-line
  path in SE(3).  Best for pick-and-place or when the tool path matters.

- **joint_quintic**: Solves IK once for the goal pose, then builds a
  minimum-jerk (quintic) trajectory in joint space with zero
  velocity/acceleration at both endpoints.  Faster to compute and
  avoids Cartesian singularities, but the tool-tip path is not
